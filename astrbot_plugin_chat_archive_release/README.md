...
1. 获取 release 包并放入 AstrBot 插件目录。

   如果您使用本 release 仓库，请复制或软链接 `astrbot_plugin_chat_archive_release/` 到 AstrBot 的插件目录，并将目录名保持为 `astrbot_plugin_chat_archive`：
   ```bash
   cd /path/to/release
   cp -a astrbot_plugin_chat_archive_release /path/to/AstrBot/data/plugins/astrbot_plugin_chat_archive
